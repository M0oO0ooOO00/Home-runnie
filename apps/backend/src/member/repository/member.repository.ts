...
export class MemberRepository {
  ...
  async createProfile(memberId: number, nickname: string, supportTeam: Team): Promise<ProfileType> {
  ...
  async findOneById(id: number): Promise<MemberType | null> {
    const result = await this.db.select().from(Member).where(eq(Member.id, id)).limit(1);
  ...
  async findOneByEmailAndProvider(
    ...
    const result = await this.db
      .select()
      .from(Member)
      .where(and(eq(Member.email, email), eq(Member.oauthProvider, provider)))
      .limit(1);
  ...
  async countParticipatedRecruitmentsByMemberId(memberId: number): Promise<number> {
    const result = await this.db
      .select({ count: count() })
      .from(Participation)
      .innerJoin(RecruitmentDetail, eq(Participation.recruitmentDetailId, RecruitmentDetail.id))
