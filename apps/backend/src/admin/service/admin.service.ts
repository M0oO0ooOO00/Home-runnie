...
export class AdminService {
  ...
  async getMemberDetail(memberId: number): Promise<MemberDetailResponseDto> {
    const memberDetailData = await this.memberService.findMemberDetailForAdmin(memberId);

    return MemberDetailResponseDto.from(
      ...
      memberDetailData.warnRecords.map((record) => ({
        ...
        post: record.post
          ? {
              ...record.post,
              postType: this.mapPostTypeFromString(record.post.post_type),
            }
          : null,
      })),
  ...
  private mapPostTypeFromString(postTypeString: string): PostType {
    switch (postTypeString) {
      ...
      default:
        throw new BadRequestException('게시글 타입이 올바르지 않습니다.');
    }
  }
}
