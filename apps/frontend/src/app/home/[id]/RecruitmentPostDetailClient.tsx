...
import type { GetRecruitmentPostDetailResponse } from '@/apis/post/post';
...
export default function RecruitmentPostDetailClient({
  initialPost,
}: {
  initialPost?: GetRecruitmentPostDetailResponse | null;
}) {
  const router = useRouter();
  const queryClient = useQueryClient();
  const params = useParams<{ id: string }>();
  const postId = Number(params.id);
  const { data, isLoading, isError } = useRecruitmentPostDetailQuery(postId, initialPost);
