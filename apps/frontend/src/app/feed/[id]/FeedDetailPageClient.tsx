...
export default function FeedDetailPageClient({
  params,
  initialPost,
}: FeedDetailPageProps & {
  ...
  const { data: post, isLoading, isError, error } = useFeedPostQuery(postId, initialPost);
    ...
    <div className="mx-auto max-w-[680px] px-4 py-4 sm:px-6">
      ...
      {post && (
        <>
          <FeedCard
            post={post}
            priorityImage={Boolean(post.images[0])}
