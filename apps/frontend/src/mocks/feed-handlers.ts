...
function generateMockPost(id: number): FeedPost {
  ...
  return {
    ...
    author: {
      type: 'member',
      id: 1000 + id,
      nickname: `${NICKNAMES[id % NICKNAMES.length]}${id}`,
      supportTeam: team,
    },
...
const ALL_POSTS: FeedPost[] = Array.from({ length: TOTAL_POSTS }, (_, i) =>
  generateMockPost(TOTAL_POSTS - i),
);
...
export const feedHandlers = [
  http.get('*/api/post/feed', async ({ request }) => {
    ...
    if (cursor) {
      ...
      if (cursorId !== null) {
        const idx = ALL_POSTS.findIndex((p) => p.id === cursorId);
        startIdx = idx >= 0 ? idx + 1 : 0;
      }
    }

    const items = ALL_POSTS.slice(startIdx, startIdx + limit);
    const lastItem = items[items.length - 1];
    const hasMore = startIdx + limit < ALL_POSTS.length;
